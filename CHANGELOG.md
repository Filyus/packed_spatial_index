...
- **AVX-512 search and raycast collect hits in a register, so they no longer
  ...
  packed its hits with `VPCOMPRESSQ` straight to memory, a form Zen 4 runs in
  microcode at about 142 cycles an instruction. They now compress into a
  register and store the whole vector. On a Zen 4 (EPYC 9V74) range search gets
  1.04–2.1× faster and all-hits raycast 2.0–2.3×: `SimdIndex2D`, which took
  1.10× the scalar `Index2D`'s time on small windows there and 1.27× on large,
  now takes 0.61× and 0.73×. Other CPUs are unchanged; on a Zen 5 laptop the
  two forms measure within 3% of each other.
...
- **2D range search on aarch64 goes back to the per-child branch.** The owned
  `Index2D`, `Index2DView` and scalar `Index2DF32` collect paths (`search`,
  `search_into`, `search_with`, `count` on f32) fold child tests into a
  bitmask, which wins 10–27% on x86 for the `f64` paths but lost 2–17% on a
  Neoverse N2 on every window size. On aarch64 they now test one child at a
  time; x86 keeps the mask, and 3D keeps it everywhere.
...
- **The portable SIMD search kernel builds one mask per internal node before it
  ...
  Scope, measured after the fact and worth stating plainly: this is the `wide`
  tier, which runtime dispatch reaches only with neither AVX-512 nor AVX2 — so
  the rows above describe the kernel that runs on aarch64 and pre-AVX2 x86, not
  the one a current x86_64 box selects, where `search_into` is about twice as
  fast before any of this. Porting the same shape to the two intrinsic tiers was
  measured and does **not** ship: about -5% on large windows and +5% on small
  ones in AVX-512, opposite signs on the commoner query shape. See
  `docs/performance.md` and the `probe/simd-tier-shapes` branch.
