...
fn distance(a: Box2D, b: Box2D) -> f64 {
...
fn naive_pairs_to(a: &[Box2D], b: &[Box2D], k: usize, max_distance: f64) -> Vec<Pair> {
    let mut all = Vec::new();
...
fn naive_pairs(boxes: &[Box2D], k: usize, max_distance: f64) -> Vec<Pair> {
    let mut all = Vec::new();
...
fn scenes(seed: u64) -> Vec<(Vec<Box2D>, Vec<Box2D>)> {
    ...
    for _ in 0..3 {
        ...
        out.push((grid_boxes(&mut rng, 80, 8), grid_boxes(&mut rng, 50, 8)));
    }
    out
}
...
fn closest_pairs_to_match_brute_force() {
    for (boxes_a, boxes_b) in scenes(5102) {
        for node_size in [2, 4, 16] {
...
fn within_forms_match_brute_force() {
    for (boxes_a, boxes_b) in scenes(5103) {
...
fn k_is_a_prefix_of_k_plus_one() {
    for (boxes_a, boxes_b) in scenes(5104) {
        let a = build(&boxes_a, 4);
        let b = build(&boxes_b, 4);
...
fn view_matches_owned() {
    for (boxes_a, boxes_b) in scenes(5106) {
        let a = build(&boxes_a, 4);
        let b = build(&boxes_b, 4);
...
mod simd {
    ...
    fn build_simd(boxes: &[Box2D], node_size: usize) -> SimdIndex2D {
        let mut builder = Index2DBuilder::new(boxes.len()).node_size(node_size);
        for &b in boxes {
            builder.add(b);
        }
    ...
    fn simd_and_view_match_brute_force() {
        for (boxes_a, boxes_b) in scenes(5107) {
