...
fn naive_closest_pair_to(a: &[Box3D], b: &[Box3D]) -> Option<f64> {
...
fn naive_closest_pair(boxes: &[Box3D]) -> Option<f64> {
    let mut best = f64::INFINITY;
...
fn check_pair(a: &[Box3D], b: &[Box3D], found: Option<(usize, usize, f64)>, expected: Option<f64>) {
...
fn closest_pair_to_matches_brute_force() {
    let mut rng = StdRng::seed_from_u64(5201);
    for (n, m, max_size) in [
        (0, 5, 4.0),
        (5, 0, 4.0),
        (1, 1, 4.0),
...
fn closest_pair_matches_brute_force() {
    let mut rng = StdRng::seed_from_u64(5202);
    for (n, max_size) in [
        (0, 4.0),
        (1, 4.0),
        (2, 4.0),
...
fn single_item_index_has_no_pair() {
    let index = build(&[Box3D::new(0.0, 0.0, 0.0, 1.0, 1.0, 1.0)]);
...
fn overlapping_boxes_are_zero_apart() {
    let boxes = [
        Box3D::new(0.0, 0.0, 0.0, 2.0, 2.0, 2.0),
        Box3D::new(1.0, 1.0, 1.0, 3.0, 3.0, 3.0),
        Box3D::new(90.0, 90.0, 90.0, 91.0, 91.0, 91.0),
    ];
    let index = build(&boxes);
...
fn view_matches_owned() {
    let mut rng = StdRng::seed_from_u64(5203);
...
fn closest_pair_is_symmetric_in_distance() {
    let mut rng = StdRng::seed_from_u64(5204);
    for _ in 0..8 {
...
mod simd {
    ...
    fn simd_and_view_match_brute_force() {
        let mut rng = StdRng::seed_from_u64(5206);
