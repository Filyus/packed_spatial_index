...
1. `Release 1: prepare version PR` prepares a draft release PR with the version
   bump and `CHANGELOG.md` update.
2. `Release 2: publish crate` publishes the reviewed version and creates the
...
2. Run the `Release 1: prepare version PR` workflow from `main` with
   `dry_run: true`.
...
4. If the dry run looks right, run `Release 1: prepare version PR` again with
   `dry_run: false`.
...
8. Run the `Release 2: publish crate` workflow from `main`.
...
4. Revoke the token.
5. Run the `First release: create tag` workflow from `main`:
...
This is required so `Release 1: prepare version PR` can create and update draft
release PRs with `GITHUB_TOKEN`.

### Release Environment Approval

The `Release 2: publish crate` workflow uses `environment: release` for the real
