...
impl SimdIndex3D {
    ...
    /// [`Index3D::aggregate`](crate::Index3D::aggregate); returns `None` when the
    ...
    /// [`SimdIndex3D::join_within`] needs an `max_distance` and reports every pair inside
    ...
    /// that are at most `max_distance` apart. See [`crate::Index3D::closest_pairs`].
    ...
    /// See [`crate::Index3D::closest_pairs_to`].
    pub fn closest_pairs_to(&self, other: &SimdIndex3D, k: usize) -> Vec<(usize, usize, f64)> {
        closest_pairs_to_core(self, other, k, f64::INFINITY)
    }

    /// Return up to `k` closest pairs of items between this index and `other`
    /// that are at most `max_distance` apart. See [`crate::Index3D::closest_pairs_to`].
...
impl<'a> SimdIndex3DView<'a> {
    ...
    /// `max_distance` of `query`. See [`SimdIndex3D::search_within`].
    pub fn search_within(&self, query: Box3D, max_distance: f64) -> Vec<usize> {
        let mut out = Vec::new();
        self.search_within_into(query, max_distance, &mut out);
        out
...
impl SimdIndex3DView<'_> {
    ...
    ///
    /// See [`Index3D::raycast_any`](crate::Index3D::raycast_any): a depth-first
    ...
    /// hit mask [`Index3DView::raycast_any`](crate::Index3DView::raycast_any)
