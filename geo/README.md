...
```toml
[dependencies]
packed_spatial_index_geo = { version = "0.26", default-features = false, features = ["async"] }
```
...
For deterministic object-storage pagination, artifacts whose manifest says
entries do not duplicate source rows can use
[`search_payload_headers_page_async`][search_payload_headers_page_async]. It
returns a [`GeoPayloadHeaderPage`][GeoPayloadHeaderPage] with the exact match
count while retaining only the requested entry-id prefix for single-box and
...
```text
gp2psindex join towers.psi cables.psi --within 500
{"a":0,"b":17}
{"a":0,"b":22}

gp2psindex join towers.psi towers.psi --within 50 --count
1841
```

The distance is box-to-box Euclidean in the artifacts' own coordinate units:
zero when the boxes overlap, inclusive at the bound, so `--within 0` is the
