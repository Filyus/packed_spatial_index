...
impl SimdIndex2DF32 {
    ...
    pub fn count(&self, query: Box2D) -> usize {
        let mut stack = crate::traversal::ScratchStack::take();
        self.count_wide(Box2DF32::from_box2d_inward(query), &mut stack)
    }
    ...
    fn count_wide(&self, q: Box2DF32, stack: &mut Vec<usize>) -> usize {
        use wide::f32x8;
...
impl<'a> SimdIndex2DF32View<'a> {
    ...
    fn count_hits(&self, query: Box2D, stack: &mut Vec<usize>) -> usize {
        stack.clear();
        if self.num_items == 0 {
            return 0;
        }
...
impl Index2DF32 {
    ...
    fn collect_node<const MASKED: bool>(
        ...
        while start < end {
            ...
            let boxes = || {
                min_xs.iter().zip(min_ys).zip(max_xs).zip(max_ys).map(
                    |(((&min_x, &min_y), &max_x), &max_y)| Box2DF32 {
                        min_x,
                        min_y,
                        max_x,
                        max_y,
            ...
            if MASKED {
                ...
                for_each_hit(mask, |i| {
                    take(i, Box2DF32::from_soa(min_xs, min_ys, max_xs, max_ys, i))
                });
            } else {
