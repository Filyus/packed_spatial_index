...
impl Index3D {
    ...
    /// `epsilon = 0.0` answers exactly [`Index3D::join`].
    ///
    /// # Example
    ///
    /// ```
    ...
    /// first.
    ///
    /// The bulk form of [`Index3D::neighbors_of_box`]: the same answer that loop
    ...
    /// ```
    pub fn knn_join(&self, other: &Index3D, k: usize) -> Vec<Vec<usize>> {
...
impl<'a> Index3DView<'a> {
    ...
    /// `epsilon` of `query`. See [`Index3D::search_within`].
    pub fn search_within(&self, query: Box3D, epsilon: f64) -> Vec<usize> {
        let mut out = Vec::new();
        self.search_within_into(query, epsilon, &mut out);
        out
    ...
    /// it. See [`Index3D::knn_join`].
    pub fn knn_join(&self, other: &Index3DView<'_>, k: usize) -> Vec<Vec<usize>> {
        knn_join_core(self, other, k)
    }
