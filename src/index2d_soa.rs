...
impl SimdIndex2D {
    ...
    /// first.
    ///
    /// The bulk form of [`SimdIndex2D::neighbors_of_box`]: the same answer that loop
    ...
    /// ```
    pub fn knn_join(&self, other: &SimdIndex2D, k: usize) -> Vec<Vec<usize>> {
...
impl<'a> SimdIndex2DView<'a> {
    ...
    /// `epsilon` of `query`. See [`SimdIndex2D::search_within`].
    pub fn search_within(&self, query: Box2D, epsilon: f64) -> Vec<usize> {
        let mut out = Vec::new();
        self.search_within_into(query, epsilon, &mut out);
        out
    ...
    /// it. See [`SimdIndex2D::knn_join`].
    pub fn knn_join(&self, other: &SimdIndex2DView<'_>, k: usize) -> Vec<Vec<usize>> {
        knn_join_core(self, other, k)
    }
