...
use packed_spatial_index_geo::{
    Box2D, BuildOpts, ConvertOpts, GeoError, GeometryColumnSelection, GeometryMetadataSource,
    GeometrySelectionReason, ReadOpts, SliceReader, StreamIndex2D, build_index_2d, convert_2d,
    decode_row_wkb_payload, discover, discover_with_opts, inspect, read_bboxes_2d,
...
fn one_native_3d_geometry_column_without_stats() -> Bytes {
    let batch = RecordBatch::try_from_iter(vec![(
        "geometry",
        binary_col(&[wkb_point_3d(1.0, 2.0, 3.0)]),
    )])
    ...
    let options = ArrowWriterOptions::new()
        ...
        .with_parquet_schema(native_geometry_schema(&["geometry"]));
    let mut buf = Vec::new();
    let mut writer = ArrowWriter::try_new_with_options(&mut buf, batch.schema(), options).unwrap();
    writer.write(&batch).unwrap();
    writer.close().unwrap();
...
#[test]
fn discover_selects_single_native_geometry_column() {
    let discovery = discover(srid_fixture()).unwrap();
    assert_eq!(discovery.num_rows, 1);
    assert_eq!(discovery.geo_metadata_version, None);
    assert_eq!(discovery.geo_primary_column, None);
    assert_eq!(
        discovery.selection,
    ...
    assert_eq!(column.encoding, "GEOMETRY");
    assert_eq!(column.crs.as_deref(), Some("srid:5070"));
...
fn explicit_geometry_column_disambiguates_native_columns() {
    ...
    let discovery = discover(data.clone()).unwrap();
    assert_eq!(
        discovery.selection,
        GeometryColumnSelection::Ambiguous {
            columns: vec!["geom_a".to_string(), "geom_b".to_string()],
        }
    );
    ...
    assert_eq!(
        discover_with_opts(data.clone(), read_opts.clone())
            .unwrap()
            .selection,
        GeometryColumnSelection::Selected {
            column: "geom_b".to_string(),
...
fn explicit_missing_geometry_column_errors_clearly() {
    ...
    let build_err = match build_index_2d(
        srid_fixture(),
        BuildOpts {
            geometry_column: Some("missing".to_string()),
            ..Default::default()
        ...
        Ok(_) => panic!("expected missing geometry column"),
        Err(err) => err,
    };
    assert!(matches!(build_err, GeoError::GeometryColumnNotFound(name) if name == "missing"));

    let convert_err = convert_2d(
        srid_fixture(),
        ConvertOpts {
            geometry_column: Some("missing".to_string()),
            ..Default::default()
    ...
    .unwrap_err();
    assert!(matches!(convert_err, GeoError::GeometryColumnNotFound(name) if name == "missing"));
}

#[test]
fn discover_keeps_unknown_native_dims_metadata_only() {
    let data = one_native_3d_geometry_column_without_stats();
    let discovery = discover(data.clone()).unwrap();
    assert_eq!(discovery.columns[0].dims, None);
...
#[test]
fn cli_inspect_json_reports_native_fixture() {
    let input = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests")
        .join("fixtures")
        .join("parquet-geospatial")
        .join("crs-srid.parquet");
    ...
    assert_eq!(json["columns"][0]["name"], "geometry");
    assert_eq!(json["selection"]["status"], "selected");
    assert_eq!(json["selection"]["reason"], "single_native_parquet");
    assert_eq!(json["columns"][0]["metadata_source"], "parquet_geospatial");
}
