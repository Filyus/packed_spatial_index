...
impl SimdIndex2D {
    ...
    pub fn join_each<B, F>(&self, other: &SimdIndex2D, visitor: F) -> ControlFlow<B>
    where
    ...
    /// [`search_within_any`](SimdIndex2D::search_within_any).
    ///
    /// # Example
    ///
    /// ```
    ...
    fn collect_within<F>(&self, query: Box2D, max_distance: f64, emit: F)
    where
        F: FnMut(usize),
    {
        let mut stack = crate::traversal::ScratchStack::take();
    ...
    /// ```
    pub fn closest_pair_to(&self, other: &SimdIndex2D) -> Option<(usize, usize, f64)> {
    ...
    ///
    /// See [`SimdIndex2D::closest_pair_to`] for the distance semantics. An item is never
...
impl<'a> SimdIndex2DView<'a> {
    ...
    /// Visit every intersecting pair between `self` and `other`. See
    /// [`Index2D::join_each`](crate::Index2D::join_each).
    pub fn join_each<B, F>(&self, other: &SimdIndex2DView<'_>, visitor: F) -> ControlFlow<B>
    where
        F: FnMut(usize, usize) -> ControlFlow<B>,
    {
        join_core(self, other, OverlapTest, visitor)
    ...
    /// [`SimdIndex2D::closest_pair_to`].
    pub fn closest_pair_to(&self, other: &SimdIndex2DView<'_>) -> Option<(usize, usize, f64)> {
        closest_pair_to_core(self, other)
    }
...
impl SimdIndex2DView<'_> {
    ...
    pub fn raycast_each<B, F>(&self, ray: Ray2D, mut visitor: F) -> ControlFlow<B>
    where
        F: FnMut(usize, f64) -> ControlFlow<B>,
    {
        let mut queue = BinaryHeap::with_capacity(DEFAULT_NEIGHBOR_QUEUE_CAPACITY);
