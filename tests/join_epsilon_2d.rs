...
fn naive_epsilon_join(a: &[Box2D], b: &[Box2D], epsilon: f64) -> BTreeSet<(usize, usize)> {
...
fn naive_self_epsilon_join(boxes: &[Box2D], epsilon: f64) -> BTreeSet<(usize, usize)> {
    let mut out = BTreeSet::new();
...
fn naive_anti_join(a: &[Box2D], b: &[Box2D], epsilon: f64) -> BTreeSet<usize> {
...
fn naive_components(boxes: &[Box2D], epsilon: f64) -> Vec<usize> {
...
fn epsilon_boundary_is_inclusive() {
    // Exactly epsilon apart on x, overlapping spans on y.
...
fn view_epsilon_family_matches_owned() {
    ...
    for epsilon in [0.0, 2.5] {
        ...
        assert_eq!(
            view_a.anti_join_epsilon(&view_b, epsilon),
            a.anti_join_epsilon(&b, epsilon),
            "eps={epsilon}"
        );
    }
    assert_eq!(
