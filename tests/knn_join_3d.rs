...
fn naive_knn_join(a: &[Box3D], b: &Index3D, k: usize) -> Vec<Vec<usize>> {
...
fn row_distances(query: Box3D, boxes: &[Box3D], row: &[usize]) -> Vec<f64> {
    row.iter()
        .map(|&i| query.distance_to_box(boxes[i]))
        .collect()
}

fn assert_matches_naive(a_boxes: &[Box3D], b_boxes: &[Box3D], k: usize) {
    let b = build(b_boxes);
    let a = build(a_boxes);
...
fn degenerate_inputs_give_empty_rows() {
    ...
    let a = build(&a_boxes);
    let b = build(&[Box3D::new(2.0, 2.0, 2.0, 3.0, 3.0, 3.0)]);
    let empty = build(&[]);
...
fn rows_are_indexed_by_item_id_not_traversal_order() {
    ...
    let a_boxes = [
        Box3D::new(90.0, 90.0, 90.0, 91.0, 91.0, 91.0), // id 0, far from b0
        Box3D::new(0.0, 0.0, 0.0, 1.0, 1.0, 1.0),       // id 1, on top of b0
    ];
    let a = build(&a_boxes);
    let b = build(&[
        Box3D::new(0.0, 0.0, 0.0, 1.0, 1.0, 1.0),
        Box3D::new(89.0, 89.0, 89.0, 90.0, 90.0, 90.0),
    ]);
...
fn self_join_reports_the_item_itself_first() {
    ...
    let boxes = [
        Box3D::new(0.0, 0.0, 0.0, 1.0, 1.0, 1.0),
        Box3D::new(10.0, 0.0, 0.0, 11.0, 1.0, 1.0),
        Box3D::new(50.0, 0.0, 0.0, 51.0, 1.0, 1.0),
    ];
...
mod simd {
    ...
    fn simd_and_view_match_the_scalar_oracle() {
        let mut rng = StdRng::seed_from_u64(7105);
